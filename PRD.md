...
Technically-minded people who want to build production RAG systems using AI coding tools (Z.ai, Cursor, etc.). They don't need to know Python or React - that's the AI's job.
...
| LLM | Ollama (ministral-3:8b-cloud) |
| Observability | RAGAS |

## Constraints

- No LLM frameworks - raw OpenAI-compatible SDK (via Ollama endpoint) using the standard Chat Completions API, Pydantic for structured outputs
...
**Build:** Auth, chat UI, Ollama integration (ministral-3:8b-cloud) via Chat Completions API, RAGAS evaluation setup.

**Learn:** Local model setup, prompt engineering, Chat Completions API basics, observability, and the benefits of owning your inference stack.
